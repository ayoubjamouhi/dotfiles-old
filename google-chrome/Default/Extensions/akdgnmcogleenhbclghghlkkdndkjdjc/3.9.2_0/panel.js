...
var Panel = function (_Container) {
  ...
  _createClass(Panel, [{
    ...
    value: function closeHandler() {
      ...
      this.sendMessage('sq.setConfigurationItem', { name: 'core.changelog_shown', value: '3.9.2' }).then(function () {
        _this2.hide();
    ...
    value: function addChangelog() {
      ...
      Promise.all(texts).then(function (lines) {
        lines[0] = lines[0].replace('{version}', '3.9.2').replace('{date}', '3/23/2021');
        _this3.addChangeHeader(lines[0]);

        _this3.addChangeFix('Fixed indexing on Google SERP.');
        _this3.addChangeFix('Deleted Webarchive Age.');
    ...
    get: function get() {
      ...
      return new Promise(function (resolve, reject) {
        _this5.sendMessage('sq.getConfigurationItem', 'core.changelog_shown').then(function (result) {
          if (versionCompare(result, '3.9.2') !== 1) {
...
var TrafficAnalyticsReport = function (_PanelReport) {
  ...
  _createClass(TrafficAnalyticsReport, [{
    ...
    value: function process() {
      ...
      this.stateLoading();
      this._api.getTrafficAnalytics(this._urlData.clean_domain, this._urlData.clean_domain !== this._urlData.topdomain).then(this.processDataReady).catch(this.processRequestError);
    }
  }, {
    key: 'replaceURLPlaceholders',
    value: function replaceURLPlaceholders(line) {
    ...
    value: function fillTotalData(data) {
      ...
      if (typeof data.pages !== 'undefined') {
        dom.text(this._pagesVisitsRate, data.pages.toLocaleString([], { maximumFractionDigits: 2 }));
      }
    ...
    value: function fillSourceData(data) {
      ...
      if (!data.hasOwnProperty('items') || data.items === null || data.items.length === 0) {
        for (var i = 0; i < 5; i++) {
          var row = tbody.rows[i];
          dom.text(row.cells[0], '...');
          dom.setContent(row.cells[1], TrafficAnalyticsReport.percentBar(0));
          dom.text(row.cells[2], '...');
        }
      ...
      data.items.forEach(function (item, index) {
        ...
        var row = tbody.rows[index];
        dom.text(row.cells[0], _this4.t('sqPanel_traffic_analytics_source_' + item.type));
        dom.text(row.cells[2], (item.traffic * 100).toLocaleString([], { maximumFractionDigits: 2 }) + '%');
        dom.setContent(row.cells[1], TrafficAnalyticsReport.percentBar(Math.round(item.traffic * 100)));
      });
    }
  }, {
    key: 'stateConnect',
